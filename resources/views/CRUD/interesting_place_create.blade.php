    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('interesting_places.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="place_type_id" class="w-full border-gray-300 rounded">
                            @foreach($placeTypes as $type)
                                <option value="{{ $type->id }}" {{ old('place_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
